...
# modify_config.py의 TARGET_VALUES와 동일하게 유지 (수정 시 반드시 함께 동기화)
EXPECTED = {
    "core-site.xml": {
...
def fetch_daemon_conf(host, port):
    """
    /conf 엔드포인트에서 XML을 받아 {property_name: value} 딕셔너리로 파싱.
    substring 매칭(다른 프로퍼티 값과 우연히 겹치는 오탐)을 피하기 위해
    modify_config.py와 동일하게 ElementTree로 정확히 파싱한다.
    """
    try:
        resp = requests.get(f"http://{host}:{port}/conf", timeout=5)
...
def check_config_values():
    ...
    /conf는 XML로 정확히 파싱해서 프로퍼티 이름 단위로 비교 (substring 매칭 금지).
    """
    print("\n=== 1. 설정값 검증 ===")
    results = []
    ...
    nn_conf = fetch_daemon_conf(NAMENODE_HOST, NAMENODE_WEB_PORT)
    rm_conf = fetch_daemon_conf(NAMENODE_HOST, RM_WEB_PORT)
    merged_conf = {**nn_conf, **rm_conf}

    if not merged_conf:
        print("  FAIL: /conf 엔드포인트 조회 또는 파싱 실패")
    else:
        for filename, props in EXPECTED.items():
            for key, expected_value in props.items():
                actual = merged_conf.get(key)
                status = "PASS" if actual == expected_value else "FAIL"
                print(f"  [{status}] (daemon /conf) {key} -> {actual} (expected {expected_value})")
