...
def load_data(spark, input_path):
    if input_path.endswith(".parquet"):
        ...
        header = lines.first()
        return lines.filter(lambda line: line != header) \
...
def clean_and_transform(rdd, start_date, end_date):
...
def compute_daily(rdd):
    ...
    return daily_trip, daily_sales


def save_results(daily_trip, daily_sales, summary_rdd):
...
def main():
    ...
    rdd = load_data(spark, input_path)

    start_date = datetime.date(2026, 5, 1)
    end_date = datetime.date(2026, 5, 31)
    rdd = clean_and_transform(rdd, start_date, end_date)

    total_trip, total_sales, avg_distance = compute_summary(rdd)
    print(f"total_trip = {total_trip}")
    print(f"total_sales = {total_sales}")
    print(f"avg_distance = {avg_distance}")

    daily_trip, daily_sales = compute_daily(rdd)
    ...
    save_results(daily_trip, daily_sales, summary_rdd)
